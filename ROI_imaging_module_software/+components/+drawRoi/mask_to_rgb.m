function color_mask = mask_to_rgb(mask,colormaps)
    ...
    for i = 1:max(mask(:))
        ...
        color_index = mod(i - 1, num_color) + 1; % Calculate color index using modulo operator
        random_rgb_color = colormaps(color_index,:);

        color_mask(roi_3D) = repmat(random_rgb_color,sum(roi_position,'all'),1); 
    end
end