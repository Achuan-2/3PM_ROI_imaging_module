...
if 'GPU' in globals() and 'denoise_model' in globals():
    ...
    opt.patch_x = int(globals().get('patch_x', 128))
    opt.patch_t = int(globals().get('patch_t', 128))
    opt.overlap_factor = float(globals().get('overlap_factor', 0.5))
    opt.scale_factor = int(globals().get('scale_factor', 1))
    opt.test_datasize = int(globals().get('test_datasize', 1000000))
    opt.datasets_path = globals().get('datasets_path', './datasets')
...
if not model_list:
    raise FileNotFoundError(f"No files found in model path: {model_path}")
...
if opt.input is not None:
    ...
    if opt.output is None:
        raise ValueError("--output must be specified when using --input")
    
    img_list = [os.path.basename(opt.input)]
    opt.datasets_path = os.path.dirname(opt.input)
    opt.datasets_folder = ''
    ...
    output_path1 = output_dir if output_dir else '.'
    ...
    if not os.path.exists(im_folder):
        raise FileNotFoundError(f"Datasets folder not found: {im_folder}")
            ...
            for N in range(len(img_list)):
                ...
                with torch.no_grad():
                    for iteration, (noise_patch, single_coordinate) in enumerate(testloader):
                        noise_patch = noise_patch.float().cuda()  # 确保数据类型为 float32
